...
#' @export
markovian_lr_test <- function(topic_indicators, ntopics, beta) {
    .check_input_markovian_lr_test(topic_indicators, ntopics, beta)
    compute_markovian_lr_test_cpp(topic_indicators, ntopics, beta)
}

.check_input_markovian_lr_test <- function(topic_indicators, ntopics, beta) {
    checkr::assert_integer(topic_indicators, lower=0, upper=ntopics - 1)
    checkr::assert_integer(ntopics, len=1, lower=1)
    checkr::assert_numeric(beta, len=1, lower=0)
}
